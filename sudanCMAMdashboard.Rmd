...
selectInput(inputId = "indicator",
            ...
            choices = c("Performance: Cure rate" = "Cure Rate",
                        "Performance: Defaulter rate" = "Defaulter Rate",
                        "Performance: Death rate" = "Death Rate",
                        "Performance: Non-response rate" = "Non-response Rate"),
            selected = NULL)
...
output$performanceOverall <- renderPlotly({
  z <- xOverall() %>% 
    ...
    geom_line(mapping = aes(y = `Cure Rate`, colour = "Cure Rate",
                            text = paste("Cure Rate: ", 
                                         round(`Cure Rate`, 
                                               digits = 1), "%\n",
                                         MonthYear, sep = "")), 
              size = 1) +
    geom_line(mapping = aes(y = `Death Rate`, colour = "Death Rate",
                            text = paste("Death Rate: ",
                                         round(`Death Rate`, 
                                               digits = 1), "%\n",
                                         MonthYear, sep = "")), 
              size = 1) +
    geom_line(mapping = aes(y = `Defaulter Rate`, colour = "Defaulter Rate",
                            text = paste("Defaulter Rate: ", 
                                         round(`Defaulter Rate`,
                                               digits = 1), "%\n", 
                                         MonthYear, sep = "")), 
              size = 1) +
    geom_line(mapping = aes(y = `Non-response Rate`, colour = "Non-response Rate",
                            text = paste("Non-response rate: ",
                                         round(`Non-response Rate`,
                                               digits = 1), "%\n", 
                                         MonthYear, sep = "")), 
              alpha = 0.5, size = 1) +
...
output$performance <- renderPlotly({
  z <- x() %>% 
    ...
    geom_line(mapping = aes(y = `Cure Rate`, colour = "Cure Rate",
                            text = paste("Cure Rate: ", 
                                         round(`Cure Rate`, digits = 1),
                                         "%\n", Month, " ", Year, sep = "")), 
              size = 1) +
    geom_line(mapping = aes(y = `Death Rate`, colour = "Death Rate",
                            text = paste("Death Rate: ",
                                         round(`Death Rate`, digits = 1),
                                         "%\n", Month, " ", Year, sep = "")),
              size = 1) +
    geom_line(mapping = aes(y = `Defaulter Rate`, colour = "Defaulter Rate",
                            text = paste("Defaulter Rate: ",
                                         round(`Defaulter Rate`, digits = 1),
                                         "%\n", Month, " ", Year, sep = "")),
              size = 1) +
    geom_line(mapping = aes(y = `Non-response Rate`, colour = "Non-response Rate",
                            text = paste("Non-response rate: ",
                                         round(`Non-response Rate`, digits = 1),
                                         "%\n", Month, " ", Year, sep = "")),
              alpha = 0.5, size = 1) +
    scale_colour_manual(name = "",
                        values = c("Cure Rate" = "darkgreen", 
  ...
  z <- z + 
    scale_y_continuous(limits = c(0, max(overall_admissions_defaulters()[["New Admissions"]])),
                       breaks = seq(from = 0, 
                                    ...
                                    by = 5000)) +
    scale_x_date(date_breaks = "1 year", date_minor_breaks = "1 month", date_labels = "%b") %>%
    labs(title = "Admissions over time - 2016 to 2019", x = "") +
    unicef_theme +
    #theme(axis.text.x = element_text(size = 8, angle = 90),
    #      axis.text.y = element_text(size = 8))
    theme(panel.grid.minor.x = element_line(size = 1),
          axis.text.y = element_text(size = 8))
...
This dashboard has been developed in support of the evaluation of the Sudan Community-based Management of Acute Manlnutrition by [Emmanuel Mandalazi](https://katilingban.io/author/emmanuel-mandalazi/) and [Ernest Guevarra](https://ernest.guevarra.io). This dashboard and its source code is made available under a [GPL-3 license](https://github.com/katilingban/sudanCMAM/blob/master/LICENSE.md).
