...
div(style="display: inline-block;vertical-align:top;",
    actionLink(inputId = "filters_info",
               label = "",
               icon = icon(name = "info-sign", lib = "glyphicon")))

observeEvent(input$filters_info, {
  showModal(
    modalDialog(
      HTML(
        "<h4>Select state</h4>
        ...
        <br>
        <h4>Select month</h4>
        <p>From the drop-down menu, select the month for which selected performance 
...
output$performanceYearOverall <- renderPlotly({
  z <- yearTotal() %>% 
    ggplot(mapping = aes(x = Year, y = Performance, group = Year)) +
...
output$performanceYearStateOverall <- renderPlotly({
  z <- yearStateTotal() %>%
    ggplot(mapping = aes(x = Year, y = Performance, group = State)) +
    geom_col(mapping = aes(fill = Indicator), 
             width = 0.5,
             alpha = 0.5) + 
    scale_fill_manual(name = "",
                      values = c("Non-responder" = "gray80",
                                 "Dead" = "black", 
                                 "Defaulted" = "#d73027",
                                 "Cured" = "#1a9850")) +
    scale_y_continuous(limits = c(0, 100),
...
output$performanceYearLocalityOverall <- renderPlotly({
  z <- yearLocalityTotal() %>%
    ggplot(mapping = aes(x = Year, y = Performance, group = Locality)) +
    geom_col(mapping = aes(fill = Indicator), 
             width = 0.5,
             alpha = 0.5) + 
    scale_fill_manual(name = "",
...
div(style="display: inline-block;vertical-align:top;",
    actionLink(inputId = "filters_info_2",
               label = "",
               icon = icon(name = "info-sign", lib = "glyphicon")))

observeEvent(input$filters_info_2, {
  showModal(
    modalDialog(
      HTML(
        "<h4>Select state</h4>
...
selectInput(inputId = "state_name_2",
            label = "Select state",
            choices = unique(monitoring$State),
            selected = NULL)

output$select_locality_2 <- renderUI({
  req(input$state_name_2)
  selectInput(inputId = "locality_name_2",
              label = "Select locality",
              choices = unique(monitoring$Locality[monitoring$State == input$state_name_2]),
              selected = NULL)
})

uiOutput("select_locality_2")

selectInput(inputId = "chosen_year_2",
            label = "Select year",
            choices = unique(monitoring$Year),
            selected = NULL)
  ...
  if (input$showCalendar) {
    z <- z +
      geom_rect(mapping = aes(xmin = as.Date(start), xmax = as.Date(end),
                              ...
                              text = event),
                data = season_long() %>% filter(group == "Lean season"),
                alpha = 0.25) +
      geom_rect(mapping = aes(xmin = as.Date(start), xmax = as.Date(end),
                              ymin = 2500, ymax = 4500, fill = group,
                              text = event),
                data = season_long() %>% filter(group == "Harvest"),
                alpha = 0.25) +
      geom_rect(mapping = aes(xmin = as.Date(start), xmax = as.Date(end),
                ...
                alpha = 0.25) +
      geom_rect(mapping = aes(xmin = as.Date(start), xmax = as.Date(end),
                              ymin = 7500, ymax = 9500, fill = group,
                              text = event),
  ...
  if (input$showCalendar) {
    z <- z +
      geom_rect(mapping = aes(xmin = as.Date(start), xmax = as.Date(end),
                              ...
                              text = event),
                data = season_year() %>% filter(group == "Lean season"),
                alpha = 0.25) +
      geom_rect(mapping = aes(xmin = as.Date(start), xmax = as.Date(end),
                              ymin = 3500, ymax = 6000, fill = group,
                              text = event),
                data = season_year() %>% filter(group == "Harvest"),
                alpha = 0.25) +
      geom_rect(mapping = aes(xmin = as.Date(start), xmax = as.Date(end),
                ...
                alpha = 0.25) +
      geom_rect(mapping = aes(xmin = as.Date(start), xmax = as.Date(end),
                              ymin = 10500, ymax = 12500, fill = group,
                              text = event),
  ...
  if (input$showCalendar) {
    z <- z +
      ...
      geom_rect(mapping = aes(xmin = as.Date(start), xmax = as.Date(end),
                ...
                alpha = 0.25) +
      geom_rect(mapping = aes(xmin = as.Date(start), xmax = as.Date(end),
                              ymin = 1050, ymax = 1250, fill = group,
                              text = event),
                data = season_year() %>% filter(group == "Rainfall"),
                alpha = 0.25)
  }
...
```{r}
div(style="display: inline-block;vertical-align:middle;",            
    h4("Effectiveness settings"))

div(style="display: inline-block;vertical-align:top;",
    actionLink(inputId = "effectiveness_settings_info",
               label = "",
               icon = icon(name = "info-sign", lib = "glyphicon")))

observeEvent(input$effectiveness_settings_info, {
  showModal(
    modalDialog(
      HTML(
        "<p>These settings are used for the <strong>effectiveness indicators</strong>
        section of this dashboard. The <strong>effectiveness indicators</strong>
        are:</p>
        <ul>
          <li>Median length-of-stay
          <li>Median average weight gain
        </ul>
        <br>
        <h4>Show data points</h4>
        <p>This checkbox indicates whether the <strong>effectiveness indicators</strong>
        data points should be shown overlaid on the boxplots.</p>
        <p>On dashboard startup, the checkbox is unchecked and data points are 
        not shown on the boxplots.</p>
        "
      ),
      title = "Effectiveness settings",
      footer = modalButton("Close")
    )
  )
})
...
length_of_stay <- otp_beneficiaries %>%
  dplyr::filter(!is.na(admDate), !is.na(disDate)) %>%
...
output$los_state <- renderPlotly({
  if (input$show_data) {
    los_state_plot <- length_of_stay %>%
      ...
      geom_boxplot(width = 0.3, colour = unicef_blue) +
    ...
    xx <- ggplotly(p = los_state_plot)
    xx$x$data[[1]]$marker$opacity <- 0
  } else {
    los_state_plot <- length_of_stay %>%
      filter(los >= 0) %>%
      ggplot(mapping = aes(x = state, y = los)) +
    ...
    xx <- ggplotly(p = los_state_plot)
    xx$x$data[[1]]$marker$outliercolor <- unicef_blue
    xx$x$data[[1]]$marker$line$color <- unicef_blue
  }
...
output$los_locality <- renderPlotly({
  if (input$show_data) {
    los_locality_plot <- length_of_stay %>%
      filter(los >= 0) %>%
      mutate(locality = factor(x = locality, 
                               levels = unique(length_of_stay$locality))) %>%
      ggplot(mapping = aes(x = locality, y = los)) +
      geom_boxplot(width = 0.3, colour = unicef_blue, alpha = 0.5) +
      geom_jitter(width = 0.05, 
                  colour = unicef_darkblue,
                  fill = unicef_darkblue,
                  alpha = 0.5, size = 2,
    ...
    xx <- ggplotly(p = los_locality_plot)
  
    xx$x$data[[1]]$marker$opacity <- 0
    xx$x$data[[2]]$marker$opacity <- 0
  } else {
    los_locality_plot <- length_of_stay %>%
      filter(los >= 0) %>%
      mutate(locality = factor(x = locality, 
    ...
    xx$x$data[[1]]$marker$outliercolor <- unicef_blue
    xx$x$data[[1]]$marker$line$color <- unicef_blue
    xx$x$data[[2]]$marker$outliercolor <- unicef_blue
    xx$x$data[[2]]$marker$line$color <- unicef_blue
    xx$x$data[[3]]$marker$outliercolor <- unicef_blue
    xx$x$data[[3]]$marker$line$color <- unicef_blue
  }
...
```{r wtGain3, fig.height = 5}
output$wt_gain_locality <- renderPlotly({
  if (input$show_data) {
    z <- wt_gain %>%
      mutate(locality = factor(x = locality, levels = unique(locality))) %>%
...
### Years living with disability (YLD) averted - Aljazira

```{r}
yldAljazira <- round((((182.5 - median_los_state[1, 2]) / 365) * 0.127) * 850, digits = 2)

renderValueBox({
  valueBox(
    value = floor(yldAljazira),
    caption = "Years living with disability (YLD) averted - Aljazira 2020",
    icon = "fa-heartbeat",
    color = "success"
  )
})
...
### Years living with disability (YLD) averted - Kassala

```{r}
yldKassala <- round((((182.5 - median_los_state[2, 2]) / 365) * 0.127) * 12035, digits = 2)

renderValueBox({
  valueBox(
    value = floor(yldKassala),
    caption = "Years living with disability (YLD) averted - Kassala 2020",
    icon = "fa-heartbeat",
    color = "success"
  )
})
...
### Years living with disability (YLD) averted - Nort Darfur

```{r}
yldNorthDarfur <- round((((182.5 - median_los_state[3, 2]) / 365) * 0.127) * 30322, digits = 2)

renderValueBox({
  valueBox(
    value = floor(yldNorthDarfur),
    caption = "Years living with disability (YLD) averted - North Darfur 2020",
    icon = "fa-heartbeat",
    color = "success"
  )
})
...
renderValueBox({
  valueBox(
    value = floor(lsAljazira),
    caption = "Number of lives saved - Aljazira 2020",
    icon = "fa-child",
    color = "warning"
  )
})
...
renderValueBox({
  valueBox(
    value = floor(lsKassala),
    caption = "Number of lives saved - Kassala 2020",
    icon = "fa-child",
    color = "warning"
  )
})
...
renderValueBox({
  valueBox(
    value = floor(lsNorthDarfur),
    caption = "Number of lives saved - North Darfur 2020",
    icon = "fa-child",
    color = "warning"
  )
})
...
renderValueBox({
  valueBox(
    value = floor(yllAljazira),
    caption = "Years of life lost (YLL) averted - Aljazira 2020",
    icon = "fa-gratipay",
    color = "primary"
  )
})
...
renderValueBox({
  valueBox(
    value = floor(yllKassala),
    caption = "Years of life lost (YLL) averted - Kassala 2020",
    icon = "fa-gratipay",
    color = "primary"
  )
})
...
renderValueBox({
  valueBox(
    value = floor(yllNorthDarfur),
    caption = "Years of life lost (YLL) averted - NorthDarfur 2020",
    icon = "fa-gratipay",
    color = "primary"
  )
})
...
### Disability-adjusted life years (DALY) - Aljazira

```{r}
dalyAljazira <- round(yldAljazira + yllAljazira, digits = 2)

renderValueBox({
  valueBox(
    value = floor(dalyAljazira),
    caption = "Disability adjusted life years (DALY) averted - Aljazira 2020",
    icon = "fa-procedures",
    color = "danger"
  )
})
...
### Disability-adjusted life years (DALY) - Kassala

```{r}
dalyKassala <- round(yldKassala + yllKassala, digits = 2)

renderValueBox({
  valueBox(
    value = floor(dalyKassala),
    caption = "Disability adjusted life years (DALY) averted - Kassala 2020",
    icon = "fa-procedures",
    color = "danger"
  )
})
...
### Disability-adjusted life years (DALY) - Noth Darfur

```{r}
dalyNorthDarfur <- round(yldNorthDarfur + yllNorthDarfur, digits = 2)

renderValueBox({
  valueBox(
    value = floor(dalyNorthDarfur),
    caption = "Disability adjusted life years (DALY) averted - North Darfur 2020",
    icon = "fa-procedures",
    color = "danger"
  )
})
...
x5 <- muac_admission[[5]] %>%
  pivot_longer(cols = `Alnhdua`:Golo,
               names_to = "health_facilities",
               values_to = "n")

x6 <- muac_admission[[6]] %>%
  pivot_longer(cols = `Rownda A`:`Tawila Hospital`,
               names_to = "health_facilities",
               values_to = "n")

x7 <- muac_admission[[7]] %>%
  pivot_longer(cols = `Kutum Health Centre`:Damrat,
               names_to = "health_facilities",
               values_to = "n")

x8 <- muac_admission[[8]] %>%
  pivot_longer(cols = Abudeleik:`Eid albida`,
               names_to = "health_facilities",
               values_to = "n")

x9 <- muac_admission[[9]] %>%
  pivot_longer(cols = `Abu Sunun`:Banaat,
               names_to = "health_facilities",
               values_to = "n")

x10 <- muac_admission[[10]] %>%
  pivot_longer(cols = `Wad Alhadad`:Barakat,
               names_to = "health_facilities",
               values_to = "n")

x11 <- muac_admission[[11]] %>%
  pivot_longer(cols = Rufaa:Tambool,
               names_to = "health_facilities",
               values_to = "n")

x12 <- muac_admission[[12]] %>%
  pivot_longer(cols = Alsiraiha:Giad,
               names_to = "health_facilities",
               values_to = "n")

## Concatenate
xxx <- rbind(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12)
```
...
```{r}
div(style="display: inline-block;vertical-align:middle;",            
    h4("Case-finding settings"))

div(style="display: inline-block;vertical-align:top;",
    actionLink(inputId = "filters_info_3",
               label = "",
               icon = icon(name = "info-sign", lib = "glyphicon")))

observeEvent(input$filters_info_3, {
  showModal(
    modalDialog(
      HTML(
        "<h4>Select state</h4>
        <p>From the drop-down menu, select the state for which state-specific
        <strong>MUAC-at-admission</strong> visualisation is to be shown.</p>
        <p>Selected state will determine which locality is presented in the 
        locality level <strong>MUAC-at-admission</strong>.</p>
        <p>Click on drop-down menu to see list of states and select state. Kassala
        state is selected on dashboard startup.</p>
        "
      ),
      title = "Filters",
...
selectInput(inputId = "state_name_3",
            label = "Select state",
            choices = unique(xxx$state),
            selected = NULL)
```
...
```{r muac2, fig.height = 8}
muac_state <- aggregate(n ~ state + muac, data = xxx, FUN = sum)
...
output$state_muac <- renderPlotly({
  z <- muac_state %>%
    ...
    geom_col(fill = unicef_blue, alpha = 0.5) +
...
muac_locality <- aggregate(n ~ state + locality + muac, data = xxx, FUN = sum)

median_muac_locality <- muac_locality %>%
  split(f = list(muac_locality$state, muac_locality$locality), drop = TRUE) %>%
  lapply(FUN = function(x) median(rep(x$muac, x$n), na.rm = TRUE)) %>%
  unlist()
...
output$locality_muac <- renderPlotly({
  z <- muac_locality %>%
    filter(state == input$state_name_3) %>%
    ggplot(mapping = aes(x = muac, y = n)) +
    geom_col(fill = unicef_blue, alpha = 0.5) +
    geom_vline(data = median_muac_locality %>% filter(state == input$state_name_3),
               mapping = aes(xintercept = `median muac`), 
               colour = unicef_darkblue, 
               linetype = 2, size = 0.5) +
    scale_x_reverse(breaks = seq(from = 115, to = 60, by = -5)) +
    labs(title = paste("MUAC at Admission by Locality in ",
                       input$state_name_3, " State", sep = ""),
         x = "MUAC (mm)") +
    facet_grid(rows = vars(locality), scales = "free_y") +
    unicef_theme
  
  ggplotly(p = z) %>%
    layout(legend = list(orientation = "h", y = 1.02))
})
...
div(style="display: inline-block;vertical-align:top;",
    actionLink(inputId = "filters_info_4",
               label = "",
               icon = icon(name = "info-sign", lib = "glyphicon")))

observeEvent(input$filters_info_4, {
  showModal(
    modalDialog(
      HTML(
        "<h4>Select state</h4>
        <p>From the drop-down menu, select the state for which state-specific
        <strong>time-to-travel</strong> visualisation is to be shown.</p>
        <p>Selected state will determine which locality is presented in the 
        locality level <strong>time-to-travel</strong>.</p>
        <p>Click on drop-down menu to see list of states and select state. Kassala
        state is selected on dashboard startup.</p>
        <br>
        <h4>Type of beneficiary</h4>
        <p>From the drop-down menu, select the type of beneficiary for which
        <strong>time-to-travel</strong> visualisation is to be shown.</p>
        <p>Click on drop-down menu to see list of beneficiary types and select. 
        Admissions beneficiaries are selected on dashboard startup.</p>
        <br>
        <h4>Scaling for plots</h4>
        <p>From the drop-down menu, select the type of <strong>y-axis scale</strong> 
        to use for <strong>time-to-travel</strong> plots.</p>
        <p>Click on the drop-down menu to see the choice between <strong>Free</strong>
        and <strong>Fixed</strong>. Select <strong>Free</strong> to make
        <strong>y-axis scale</strong> of mutli-faceted plots indpendent of each other.
        This is good for examining <strong>time-to-travel</strong> for each state 
        or for each locality. Select <strong>Fixed</strong> to make 
        <strong>y-axis scale</strong> of mutli-faceted plots the same for each plot. 
        This is good for comparing the magnitude of values of the time series plots 
        between states or between localities. On dashboard startup, 
        <strong>Free</strong> is selected.</p>
        "
      ),
      title = "Filters",
      footer = modalButton("Close")
...
selectInput(inputId = "state_name_4",
            label = "Select state",
            choices = unique(xxx$state),
            selected = NULL)

selectInput(inputId = "beneficiary_type",
            label = "Type of beneficiary",
            choices = unique(time_to_travel$Category),
            selected = NULL)

selectInput(inputId = "scales_2",
            label = "Scaling for plots",
            choices = c("Free" = "free_y",
                        "Fixed" = "fixed"),
            selected = "free_y")
...
```{r ttt_state, fig.height = 6}
output$ttt_state_plot <- renderPlotly({
  z <- time_to_travel %>%
    filter(Category == input$beneficiary_type) %>%
    tidyr::pivot_longer(cols = `30 or less`:`more than 120`,
                        names_to = "Time-to-travel",
                        values_to = "n") %>%
    dplyr::mutate(`Time-to-travel` = paste(`Time-to-travel`, "minutes", sep = " ")) %>%
    dplyr::mutate(`Time-to-travel` = stringr::str_replace_all(string = `Time-to-travel`, 
                                                              pattern = " minutes", 
                                                              replacement = "\nminutes")) %>%
    dplyr::group_by(State, `Time-to-travel`) %>%
    dplyr::summarise(sum(n, na.rm = TRUE), .groups = "drop") %>%
    dplyr::rename(n = `sum(n, na.rm = TRUE)`) %>%
    ggplot(mapping = aes(x = `Time-to-travel`, y = n)) +
    geom_col(fill = unicef_blue, alpha = 0.5) +
    scale_y_continuous(n.breaks = 10) +
    labs(title = paste("Time-to-travel (in minutes) by state - ", 
                       input$beneficiary_type, sep = ""),
         x = "") +
    facet_wrap(. ~ State, scales = input$scales_2) +
    unicef_theme +
...
plotlyOutput(outputId = "ttt_state_plot")
```

### Locality
 
```{r ttt_locality, fig.height = 6}
output$ttt_locality_plot <- renderPlotly({
  z <- time_to_travel %>%
    filter(State == input$state_name_4,
           Category == input$beneficiary_type) %>%
    tidyr::pivot_longer(cols = `30 or less`:`more than 120`,
                        names_to = "Time-to-travel",
                        values_to = "n") %>%
    dplyr::mutate(`Time-to-travel` = paste(`Time-to-travel`, "minutes", sep = " ")) %>%
    dplyr::mutate(`Time-to-travel` = stringr::str_replace_all(string = `Time-to-travel`, 
                                                              pattern = " minutes", 
                                                              replacement = "\nminutes")) %>%
    dplyr::group_by(State, Locality, `Time-to-travel`) %>%
    dplyr::summarise(sum(n, na.rm = TRUE), .groups = "drop") %>%
    dplyr::rename(n = `sum(n, na.rm = TRUE)`) %>%
    ggplot(mapping = aes(x = `Time-to-travel`, y = n)) +
    geom_col(fill = unicef_blue, alpha = 0.5) +
    scale_y_continuous(n.breaks = 10) +
    labs(title = paste("Time-to-travel (in minutes) by locality in ", 
                       input$state_name_4, " State - ", 
                       input$beneficiary_type, sep = ""),
         x = "") +
    facet_wrap(. ~ Locality, ncol = 2, scales = input$scales_2) +
    unicef_theme +
    theme(axis.text.x = element_text(size = 6),
...
plotlyOutput(outputId = "ttt_locality_plot")
```
...
This dashboard has four main sections - *Performance*, *Responsiveness*, *Effectiveness*, *Case-finding*, and *Access* - which can be accessed via the header tabs. The *Performance* section (shown on dashboard startup) presents analysis and visualisation of CMAM performance indicators at national, state and locality level across various time periods. The *Responsiveness* section presents analysis and visualisation of CMAM responsiveness indicators at national, state and locality level across various time periods. The *Effectiveness* section presents indicators of length-of-stay in the programme and average weight gain. These indicators assess clinical effectiveness. The *Case-finding* section shows MUAC-at-admission which is an indicator of how good the case-finding mechanism of the programme is - the closer the MUAC-at-admissions are to the admitting MUAC threshold (i.e. 115 mm) the earlier that they are found in the community and the earlier the cases are found the better their treatment outcomes are. The *Access* section presents time-to-travel analysis of various programme beneficiaries as an indicator of access to services. The dashboard has a fixed sidebar that contains various user interfaces for selecting appropriate settings for respective sections. The various settings groups include an info button which when clicked shows a modal dialog box providing information and guidance on how to choose appropriate settings.
