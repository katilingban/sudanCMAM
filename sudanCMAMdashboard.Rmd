...
div(style="display: inline-block;vertical-align:top;",
    actionLink(inputId = "general_settings_info_2",
               label = "",
               icon = icon(name = "info-sign", lib = "glyphicon")))

observeEvent(input$general_settings_info_2, {
  showModal(
    modalDialog(
      HTML(
        "<p>These settings are used across the both sections (<strong>performance</strong> 
...
output$los_locality <- renderPlotly({
  los_locality_plot <- length_of_stay %>%
    ...
    ggplot(mapping = aes(x = locality, y = los, colour = state, fill = state)) +
    geom_boxplot(width = 0.3, alpha = 0.5) +
    scale_colour_manual(name = "State", 
                        values = c(unicef_orange, unicef_purple)) +
    scale_fill_manual(values = c(unicef_orange, unicef_purple)) +
    scale_y_continuous(n.breaks = 10) +
  ...
  ggplotly(p = los_locality_plot) %>%
    layout(legend = list(orientation = "h", y = 1.07))
})
...
output$wt_gain_locality <- renderPlotly({
  z <- wt_gain %>%
    ...
    ggplot(mapping = aes(x = locality, y = wtGain, colour = state, fill = state)) +
    geom_boxplot(width = 0.3, alpha = 0.5) +
    scale_y_continuous(n.breaks = 10) +
    scale_colour_manual(name = "State",
                        values = c(unicef_orange, unicef_purple)) +
...
median_muac_locality_kassala <- median_muac_locality %>%
  filter(state == "Kassala")

row.names(median_muac_locality_kassala) <- 1:nrow(median_muac_locality_kassala)

output$locality_muac_kassala <- renderPlotly({
  z <- muac_locality %>%
    filter(state == "Kassala") %>%
    ggplot(mapping = aes(x = muac, y = n)) +
    geom_col(fill = unicef_blue, alpha = 0.5) +
    geom_vline(data = median_muac_locality_kassala,
               mapping = aes(xintercept = `median muac`), 
               colour = unicef_darkblue, 
               linetype = 2, size = 0.5) +
    scale_x_reverse(breaks = seq(from = 115, to = 60, by = -5)) +
    labs(title = "MUAC at Admission by Locality in Kassala State", x = "MUAC (mm)") +
...
plotlyOutput(outputId = "locality_muac_kassala")
```

Row
--------------------------------------------------------------------------------

### By locality in North Darfur State

```{r muac4, fig.height = 8}
median_muac_locality_northdarfur <- median_muac_locality %>%
  filter(state == "North Darfur")

row.names(median_muac_locality_northdarfur) <- 1:nrow(median_muac_locality_northdarfur)

output$locality_muac_northdarfur <- renderPlotly({
  z <- muac_locality %>%
    filter(state == "North Darfur") %>%
    ggplot(mapping = aes(x = muac, y = n)) +
    geom_col(fill = unicef_blue, alpha = 0.5) +
    geom_vline(data = median_muac_locality_northdarfur,
               mapping = aes(xintercept = `median muac`), 
               colour = unicef_darkblue, 
               linetype = 2, size = 0.5) +
    scale_x_reverse(breaks = seq(from = 115, to = 60, by = -5)) +
    labs(title = "MUAC at Admission by Locality in North Darfur State", x = "MUAC (mm)") +
    facet_grid(rows = vars(locality), scales = "free_y") +
    unicef_theme
...
plotlyOutput(outputId = "locality_muac_northdarfur")
```
...
output$ttt_state_plot <- renderPlotly({
  z <- time_to_travel %>%
    ...
    dplyr::mutate(`Time-to-travel` = stringr::str_replace_all(string = `Time-to-travel`, 
                                                              pattern = " minutes", 
                                                              replacement = "\nminutes")) %>%
    dplyr::group_by(State, Category, `Time-to-travel`) %>%
    dplyr::summarise(sum(n, na.rm = TRUE), .groups = "drop") %>%
    dplyr::rename(n = `sum(n, na.rm = TRUE)`) %>%
    ggplot(mapping = aes(x = `Time-to-travel`, y = n)) +
    geom_col(fill = unicef_blue, alpha = 0.5) +
    labs(title = "Time-to-travel (in minutes) by beneficiary type and by state",
         x = "") +
    facet_grid(rows = vars(Category), cols = vars(State), scales = "free_y") +
    unicef_theme +
...
```{r ttt_locality_kassala}
output$ttt_locality_kassala_plot <- renderPlotly({
  z <- time_to_travel %>%
    filter(State == "Kassala") %>%
    tidyr::pivot_longer(cols = `30 or less`:`more than 120`,
                        names_to = "Time-to-travel",
    ...
    dplyr::mutate(`Time-to-travel` = stringr::str_replace_all(string = `Time-to-travel`, 
                                                              pattern = " minutes", 
                                                              replacement = "\nminutes")) %>%
    dplyr::group_by(State, Locality, Category, `Time-to-travel`) %>%
    dplyr::summarise(sum(n, na.rm = TRUE), .groups = "drop") %>%
    dplyr::rename(n = `sum(n, na.rm = TRUE)`) %>%
    ggplot(mapping = aes(x = `Time-to-travel`, y = n)) +
    geom_col(fill = unicef_blue, alpha = 0.5) +
    labs(title = "Time-to-travel (in minutes) by beneficiary type and locality, Kassala State",
         x = "") +
    facet_grid(Category ~ Locality, scales = "free_y") +
    unicef_theme +
...
plotlyOutput(outputId = "ttt_locality_kassala_plot")
```
...
```{r ttt_locality_northdarfur}
output$ttt_locality_northdarfur_plot <- renderPlotly({
  z <- time_to_travel %>%
    filter(State == "North Darfur") %>%
    tidyr::pivot_longer(cols = `30 or less`:`more than 120`,
                        names_to = "Time-to-travel",
    ...
    ggplot(mapping = aes(x = `Time-to-travel`, y = n)) +
    geom_col(fill = unicef_blue, alpha = 0.5) +
    labs(title = "Time-to-travel (in minutes) by beneficiary type and locality, North Darfur State",
         x = "") +
    facet_grid(Category ~ Locality, scales = "free_y") +
    unicef_theme +
    theme(axis.text.x = element_text(size = 4),
...
plotlyOutput(outputId = "ttt_locality_northdarfur_plot")
```
